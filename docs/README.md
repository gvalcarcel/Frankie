# Documentación

Esta carpeta contiene la documentación técnica y operativa del servidor de aula.

## Documentos

- `DOC-SRV-001_Servidor_Aula_n8n.md`: documento técnico principal.
- `arquitectura.md`: arquitectura física y logica.
- `seguridad.md`: medidas de seguridad aplicadas y pendientes.
- `incidencias.md`: registro de incidencias.
- `procedimientos.md`: procedimientos de operación.
- `mantenimiento_servidor.md`: procedimiento de actualización, optimización y validación posterior.
- `evidencias/`: resultados de auditorías, mantenimientos y validaciones publicables.
- `frankie-cli/design.md`: diseño técnico de Frankie CLI.

## Norma

No incluir secretos, credenciales, tokens ni claves privadas.
